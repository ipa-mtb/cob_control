  ...
  virtual bool init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& nh) {
    ...
    if (has_model) {
      ...
      if (steer_joint) {
        ...
        max_steer_rate_ = (max_steer_rate_ != 0.0 && max_steer_rate_ < steer_joint->limits->velocity) ?
                              max_steer_rate_ :
                              steer_joint->limits->velocity;
      }
      drive_joint = model.getJoint(wheel_state_.drive_name);
      if (drive_joint) {
        nh.param("max_drive_rate", max_drive_rate_, 0.0);
        max_drive_rate_ = (max_drive_rate_ != 0.0 && max_drive_rate_ < drive_joint->limits->velocity) ?
                              max_drive_rate_ :
                              drive_joint->limits->velocity;
      }
    }
    ...
    commands_pub_.reset(
        new realtime_tools::RealtimePublisher<cob_base_controller_utils::WheelCommands>(nh, "wheel_commands", 5));
...
private:
  ...
  void updateCommand() {
    ...
    if (target_.state.rotTheta != 0) {
      ...
      a1 = std::min(std::max(a1, -0.9), 0.9);

      if (a2 < 0) {
        a2 = std::min(std::max(a2, -M_PI), -M_PI + 0.9);
      } else {
        a2 = std::min(std::max(a2, M_PI - 0.9), M_PI);
      }
